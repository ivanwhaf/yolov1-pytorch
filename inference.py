...
def draw_bbox():
    ...
    for img_name in imgs:
        print(img_name)
        img = cv2.imread(
            'dataset/NWPU VHR-10 dataset/positive image set/'+img_name)
        ...
        with open('dataset/NWPU VHR-10 dataset/ground truth/'+img_name.split('.')[0]+'.txt', 'r') as f:
            ...
            for line in lines:
                ...
                x1, y1, x2, y2 = int(
                    x1*448), int(y1*448), int(x2*448), int(y2*448)

                img = cv2.resize(img, (448, 448))

                img = cv2.rectangle(img, (x1, y1), (x2, y2), COLORS[c-1], 2)
                cv2.rectangle(img, (x1-1, y1), (x2+1, y1-22),
                              COLORS[c-1], -1, cv2.LINE_AA)
